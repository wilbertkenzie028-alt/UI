...
C.u40.__GetRootPart = function(character)
    ...
    -- V8.2 FIX: keep shared dispatcher state alive after loading.
        ...
        -- Theme system removed. Keep one fixed internal palette so existing
        -- element constructors continue to have stable colors.
        local u1233 = C._new3
        ...
        function v926.ApplySetting(p964, p965, p966)
            ...
            elseif p965 == 'Theme' then
                -- Theme switching has been removed. Keep the fixed default palette.
                u932 = 'Dark'
                u935 = u1237
                C.u139 = u1237
    ...
    assert(u1240 ~= nil, "V8.2 DEBUG: u1240 NIL")
print("[V8.2] u1240 OK:", u1240)
...
print("[V8.2] u906 =", u906, typeof(u906))
print("[V8.2] u1074 =", v1074, typeof(v1074))

if typeof(u906) ~= "Instance" then
    warn("[V8.2] Skip opening-size tween: u906 bukan Instance")
else
...
u908(1)
print("[V8.2] BEFORE TABFRAME")
print("[V8.2] u1238:", u1238)
print("[V8.2] UI/root:", UI)
    ...
    local function _isGuiObject(value)
        return typeof(value) == 'Instance' and value:IsA('GuiObject')
    end

    local function _ensureThemeDecor(instance, palette)
        if not _isGuiObject(instance) then
            return
        end

        if instance.Name ~= 'NotificationContainer' then
            if instance:IsA('Frame')
                or instance:IsA('ImageButton')
                or instance:IsA('TextButton')
                or instance:IsA('TextBox') then

                -- Element containers receive their own themed surface.
                if instance.Parent and instance.Parent:IsA('ScrollingFrame') then
                    instance.BackgroundColor3 = palette.UFB
                    if instance.BackgroundTransparency > 0.95 then
                        instance.BackgroundTransparency = 0.75
                    end
    ...
    local function _applyThemeTree(root, palette)
        if typeof(root) ~= 'Instance' or not palette then
            return
        end

        if root:IsA('GuiObject') then
            if root.Name == 'MainFrame' then
                root.BackgroundColor3 = palette.MBG
            elseif root.Name == 'Title' and root.Parent == C.UI.Frame.MainFrame then
                root.BackgroundColor3 = palette.MTG
                root.TextColor3 = C._fromRGB(255, 255, 255)
            elseif root.Name == 'TabFrame' then
                root.BackgroundColor3 = palette.MBG
            elseif root.Name == 'NotificationContainer' then
                -- Container stays transparent.
            elseif root:IsA('Frame') and root.Parent and root.Parent:IsA('ScrollingFrame') then
                root.BackgroundColor3 = palette.UFB
                if root.BackgroundTransparency > 0.95 then
                    root.BackgroundTransparency = 0.75
                end
            end
        end

        _ensureThemeDecor(root, palette)

        for _, child in ipairs(root:GetChildren()) do
            _applyThemeTree(child, palette)
        end
    end
    ...
    C.UI.SetOpen = function(self, state)
        ...
        if typeof(u1238) == 'Instance' and typeof(u1238_TabFrame) == 'Instance' then
    ...
    C.UI.Toggle = function(self)
        return self:SetOpen(not openState)
    end
    ...
    local openKeyConnection = C.u157(C._UserInputService.InputBegan, function(input, processed)
        if processed then
            return
        end
    ...
    C.UI.SetOpenKey = function(self, keyCode)
    ...
    print('[Overdrive-H V8.3] Standalone module fallbacks enabled')
    print('[Overdrive-H V8.3] Notify / Open API fixed')
